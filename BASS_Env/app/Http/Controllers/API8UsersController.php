...
class API8UsersController extends Controller
{
    public function login(Request $request)
    {
        if($request->hasHeader('Authorization-Token') && $request->header('Authorization-Token')!="" )
        {
            ...
            $validuser = API8Users::where('username',$user->getUsername())->where('password',$user->getPassword())->first();
    ...
    public function store(Request $request)
    {
        ...
        $user['authkey'] = hash("sha256",$user['username'].$user['password'].base64_decode(Variables::getAPI8SecretSalt()));

        return API8Users::create($user)->makeHidden('authkey')->toArray();
    }
    ...
    public function showkey(Request $request)
    {
        ...
        {
            $user = API8Users::where('username',$_SESSION['uname'])->first();

                if($user)
                {
                    if($request->hasHeader('Origin') && trim($request->header('Origin'))!="")
                    {
                        return response(array("flag"=>base64_decode(Variables::getAPI8Flag()),"user"=>$user), 200)
                        ->header('Content-Type', 'application/json')
